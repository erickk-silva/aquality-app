...
class ProcessadorAlertas {
    ...
    public function processarLeitura($dispositivo_id, $leitura) {
        try {
            ...
            return [
                'status' => 'erro',
                'mensagem' => 'Erro ao processar alertas: ' . $e->getMessage(),
                'timestamp' => date('Y-m-d H:i:s')
            ];
        }
    }
...
try {
    $processador = new ProcessadorAlertas($pdo);
    ...
    if (!$dispositivo_id || !$leitura) {
        throw new Exception('ID do dispositivo e dados da leitura são obrigatórios');
    }
    
    $resultado = $processador->processarLeitura($dispositivo_id, $leitura);
